...
my @pretext
    = ( 'APACHE OK', 'APACHE WARNING', 'APACHE CRITICAL', 'APACHE UNKNOWN' );
my $server = $options{'H'};
my $mon    = $options{'v'};
...
sub CheckAll(\%) {
    ...
    $text
        .= ' ('
        . scalar( keys( %{ $stati{2} } ) ) . 'c/'
        . scalar( keys( %{ $stati{1} } ) ) . 'w/'
        . scalar( keys( %{ $stati{0} } ) ) . 'o)';
...
sub get_apache_status($) {
    ...
    while ( $code =~ /<dt>([^<]+)<\/dt>/g ) {
        ...
        {
            $results{'uptime'}
                = sprintf( "%04d:%02d:%02d:%02d", $1, $2, $3, $4 );
        }
        elsif ( $line
            ...
            )
        {
            ...
            =~ /Total\s+accesses:\s+(\d+)\s+-\s+Total\s+Traffic:\s+([\d\.]+)\s+([MKGB]+)/
            )
        {
            $results{'accesses'} = $1;
            $results{'traffic'}  = $2;
